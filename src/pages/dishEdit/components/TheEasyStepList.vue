...
const generateNewSteps = () => {
  ...
  for (let i = 0, len = useAppStore.editingDish.steps.length; i < len;) {
    const item = [];
    const indexes = [];
    if (useAppStore.editingDish.steps[i].instructionType !== "stir_fry") {
      item.push(useAppStore.editingDish.steps[i]);
    } else {
      if (i === 0) {
        item.push(useAppStore.editingDish.steps[i]);
      } else if (useAppStore.editingDish.steps[i - 1].instructionType === "stir_fry") {
        item.push(useAppStore.editingDish.steps[i]);
      }
    }
    indexes.push(i);

    if (i + 1 < len && useAppStore.editingDish.steps[i + 1].instructionType === "stir_fry") {
      item.push(useAppStore.editingDish.steps[i + 1]);
      indexes.push(i + 1);
      i += 2;
    } else {
      i += 1;
